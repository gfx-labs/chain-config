...
func NetworkByAny(v any) (*Network, error) {
	...
	case int:
		return NetworkById(x)
	case int8:
		return NetworkById(int(x))
	case int16:
		return NetworkById(int(x))
	case int32:
		return NetworkById(int(x))
...
// networkByString tries to resolve a string as a CAIP-2 identifier,
// an internal name, or a numeric chain id (in that order).
